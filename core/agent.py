...
from .agent_tools import create_or_modify_order, set_address, confirm_order
...
formatted_menu = format_menu_for_instructions(menu)

chef = Agent(
    name=APP_NAME,
    model="gemini-2.0-flash", # Use a suitable Gemini model
    # description=(
    #     "You are a friendly and efficient AI voice agent for a restaurant, "
    #     "dedicated to taking customer food orders. Your primary goal is to accurately "
    #     "capture all items, quantities, and any special modifications requested by the customer. "
    #     "You use the 'create_or_modify_order' tool to manage orders in the database."
    # ),
    instruction=(
        ...
        "    'Great! Your order has been placed. Is there anything else I can help you with today?'\n"
        "4.  Do not directly mention database operations or technical details to the user."
    ),
    tools=[create_or_modify_order, set_address, confirm_order]
)
...
def ask_agent(session_id: str, user_id: str, text: str, runner: Runner=runner): # for improved efficiency
